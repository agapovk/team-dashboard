...
`docs/XLSX_FORMAT.md` (импорт), `docs/ROADMAP.md` (фазы), `docs/UI_KIT.md` (дизайн-система/UI-кит),
`docs/CODE_STYLE.md` (линт/git/CI).
...
- **Vertical Slice:** фичи — вертикальные срезы в `features/<slice>/` (UI + Server Actions + чистое `core/`), сквозные. Эталон — `features/import` (`components/` + корень + `core/`, публичный `index.ts`). `lib/` — только сквозные chokepoint'ы. Детали — `docs/ARCHITECTURE.md`.
- Server Components по умолчанию; Client Components — только под стейт/интерактив.
- Срезы не импортят друг друга напрямую — только через `lib/`/`schemas/`/`types/` или публичный `index.ts` соседнего среза.
...
По завершении фазы — компактное саммари в `docs/summary/phase-N.md` (сделано + принятые решения + грабли), чтобы вернуться при необходимости.
Полные правила кода и workflow — `docs/CODE_STYLE.md`.
