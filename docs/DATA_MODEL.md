...
> - **Внешние метрики — `notNull` (MVP).** `null` при отказе датчиков (см. LOAD_MONITORING «деградируют
>   по отдельности») в MVP **не хранится** — импорт требует полных метрик. Поддержка `null`-метрик —
>   post-MVP (ослабить `notNull` + миграция). Исключения уже сейчас: `rpe` и `sessionLoad` (`null` = пробел).
